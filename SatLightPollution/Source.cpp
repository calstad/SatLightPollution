...
int main(int argc, const char *argv[])
{
	...
	std::string TLE_filepath = "C:\\Predictions\\2021-06-16.tle";//full catalog, 3LE, space-trak.org
	...
	for (int iSat = 0; iSat < tle.size(); iSat++)
	{
		...
		{
			double sat_ICS_km[3], sat_ICS_kmps[3];//Inertial system
			int err = sgp4(tle_satrec, MJD, sat_ICS_km, sat_ICS_kmps);
			v3 sat_ICS_m;//Inertial system
...
)
{
	...
	v3 SatSun_ICS_m = sun_ICS_m - sat_ICS_m;
	double sun_satellite_range_m = norm(SatSun_ICS_m);//slant range
	double sun_satellite_R2_m2 = sun_satellite_range_m * sun_satellite_range_m;
	double sun_satellite_range_AU = sun_satellite_range_m / AU_to_m;

	//phase angle		                                     
	v3 SatStation_ICS_uv = normalize(SatStation_ICS_m);//get satellite-station unit vector		
	v3 SatSun_ICS_uv = normalize(sun_ICS_m - sat_ICS_m);//get satellite-sun unit vector		
	double phase_angle_rad = IArad(SatSun_ICS_uv, SatStation_ICS_uv);

	//calculate RSO magnitude 
	//double mag_apparent = RSO_apparent_magnitude(phase_angle_rad, slant_range_m);
	//assume 1-m diameter Lambertian sphere
	double RSO_sphere_radius_m = 0.5;
	double CrossSection_area_m2 = pi * RSO_sphere_radius_m * RSO_sphere_radius_m;
	...
	double Psi_sphere = (Cd / pi) * sphere_phase_funtion;

	//Irradiation at the ground observing point
	double Irradiation_sun = 1368.0;// [W m-2], solar constant; solar irradiation at 1AU
	double Irradiation_topo = Irradiation_sun * CrossSection_area_m2 * Psi_sphere / station_satellite_R2_m2;

	double Sun_apparent_mag = -26.74;//V, apparent magnitude of Sun
	double sat_apparent_mag = Sun_apparent_mag - 2.5 * log10(Irradiation_topo / Irradiation_sun);//absolute magnitude of RSO
